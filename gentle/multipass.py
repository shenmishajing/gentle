    ...
    for wd_idx, wd in enumerate(alignment):
        if wd.not_found_in_audio():
            ...
            if len(cur_unaligned_words) > 0:
                to_realign.append({"start": last_aligned_word, "end": wd, "words": cur_unaligned_words})
                cur_unaligned_words = []

            last_aligned_word = wd

    if len(cur_unaligned_words) > 0:
        to_realign.append({"start": last_aligned_word, "end": None, "words": cur_unaligned_words})
...
def realign(wavfile, alignment, ms, resources, nthreads = 4, progress_cb = None):
    ...
    def realign(chunk):
        ...
        k = standard_kaldi.Kaldi(resources.nnet_gpu_path, chunk_gen_hclg_filename, resources.proto_langdir)
