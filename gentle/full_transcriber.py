...
class FullTranscriber():
    ...
    def make_transcription_alignment(trans):
        ...
        words = []
        for t_wd in trans:
            word = transcription.Word(case = transcription.Word.SUCCESS, startOffset = len(transcript),
                endOffset = len(transcript) + len(t_wd.word), word = t_wd.word, alignedWord = t_wd.word,
                phones = t_wd.phones, start = t_wd.start, end = t_wd.end)
            words.append(word)
